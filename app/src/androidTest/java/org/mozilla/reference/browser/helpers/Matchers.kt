...
import org.mozilla.reference.browser.ext.recursiveChildren
...
fun last(matcher: Matcher<View>): Matcher<View> = object : BaseMatcher<View>() {
    override fun describeTo(description: Description?) {
        description?.apply {
            appendText("last view matching: ")
            matcher.describeTo(this)
        }
    }

    override fun matches(item: Any?): Boolean =
        if (item is View && matcher.matches(item)) {
            (item.rootView as ViewGroup)
                    .recursiveChildren
                    .last { view -> matcher.matches(view) } == item
        } else {
            false
        }
}
